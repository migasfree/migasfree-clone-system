...
## 📁 Key File Map
- `/build`: Orchestrates the image creation on the host.
...
- `/test`: QEMU launch script for verifying the build.
- `/test-boot`: QEMU script to verify if the cloned disk is bootable.
...
- **Building**: Always use `sudo ./build`. It creates `artifacts/mcs-<version>.iso`.
- **Testing**: Use `sudo ./test`. It simulates a real environment with a target disk `artifacts/target-hd.qcow2`.
- **Verification**: After cloning inside the test VM, use `sudo ./test-boot` to verify the target disk's bootloader.
