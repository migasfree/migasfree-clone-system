...
MCS expects a `catalog.json` file at the pool root (`http://<SERVER>/pool/mgi/catalog.json`) listing available projects. See the [Server Setup](docs/server_setup.md) guide for the format and the [Architecture](docs/architecture.md) document for details on the project directory structure.
