...
@test "clone_HD: HTTP Turbo Clone succeeds and writes SYSTEM and HOME" {
    run clone_HD "http://server/pool/mgi/project/" "/dev/sda"
    assert_success
}
...
@test "clone_HD: preserve HOME skips HOME partition" {
    run clone_HD "http://server/pool/mgi/project/" "/dev/sda" "true"
    assert_success
}
...
@test "clone_HD: returns error on HTTP source without partition.yml" {
    ...
    run clone_HD "http://server/pool/mgi/nonexistent/" "/dev/sda"
    assert_failure
}
